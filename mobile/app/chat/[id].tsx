...
import { listenConversation, leaveConversation } from "@/src/services/echo";
...
export default function ChatScreen() {
  ...
  useEffect(() => {
    if (!id) return;
    activeScreen.conversationId = Number(id);
    setLoading(true);
    messageService
    ...
    listenConversation(Number(id), (incoming: Message) => {
    ...
    return () => {
      activeScreen.conversationId = 0;
      leaveConversation(Number(id));
    };
  }, [id]);

  const confirmRemoveFriend = () => {
    Alert.alert("Remove Friend", `Delete conversation with ${name}?`, [
  ...
  // ── Send with optional files ───────────────────────────────
  const handleSend = async (text: string, files: AttachFile[]) => {
    if (!id) return;
    try {
      const msg = await messageService.sendToConversation(
        Number(id),
        text || undefined,
